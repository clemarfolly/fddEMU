...
const FatBS vfBootSector PROGMEM = {
    ...
    {0x0E, 0x1F, 0xBE, 0x5B, 0x7C, 0xAC, 0x22, 0xC0, 0x74, 0x0B, 0x56, 0xB4, 0x0E, 0xBB, 0x07, 0x00, 0xCD, 0x10, 0x5E,
     ...
     0x72, 0x79, 0x20, 0x61, 0x67, 0x61, 0x69, 0x6E, 0x20, 0x2E, 0x2E, 0x2E, 0x20, 0x0D, 0x0A, 0x00, 0x00, 0x00, 0x00,
...
#define DATA_SECTOR                                                                                                    \
    ROOT_SECTOR + (((NROOT * sizeof(DIRE)) + 511) / 512) // root_sector + ((n_root_entries*dir_entry_size +511)/512)
...
uint32_t findSDrootSector(uint8_t *buffer)
{
    ...
    if ((pBS->fsID[0 == 'F']) && (pBS->fsID[0 == 'A']))
        ...
        rootSect = bootSect + p32BS->reservedSectors + (p32BS->nFATs * p32BS->sectorsPerFAT) +
                   ((p32BS->rootCluster - 2) * p32BS->sectorsPerCluster);
#if DEBUG
...
void VirtualFloppyFS::loadImage()
{
    ...
    {
        if (flags & DRIVE0)
            drive[0].load(filename);
#if ENABLE_DRIVE_B
        else if (flags & DRIVE1)
            drive[1].load(filename);
...
void VirtualFloppyFS::genDataSector(uint8_t *buffer, uint16_t sector)
{
    ...
    if ((r_sector >= DISKS_START) && (r_sector < DISKS_END))
    {
        ...
        if (r_sector == 0)
            flags &= ~F_EOFLIST; // clear EOF flag;
        if (sdRootSect)
        {
            if (flags & F_EOFLIST)
                memset(buffer, 0, 512);
