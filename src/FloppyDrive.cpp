...
void initFDDpins()
{
    ...
    DDRB &= ~((1 << PIN_MOTORA) |
              (1 << PIN_SELECTA)); // PB0 RXLED, PB1 SCK, PB2 MOSI, PB3 MISO, PB4 MOTORA, PB5 OCP1, PB6 SELECTA
    DDRC &= ~(1 << PIN_SIDE);      // PC6 SIDE
    DDRD &= ~((1 << PIN_STEP) | (1 << PIN_STEPDIR) | (1 << PIN_READDATA) | (1 << PIN_INDEX) |
              (1 << PIN_WRITEPROT)); // PD0 SCL, PD1 SDA, PD5 TXLED, PD2 STEP, PD3 STEPDIR, PD4 ICP1, PD7 INDEX
    ...
    PORTD |= (1 << PIN_STEP) | (1 << PIN_STEPDIR) |
             (1 << PIN_READDATA);  // PD0 SCL, PD1 SDA, PD5 TXLED, PD2 STEP, PD3 STEPDIR, PD4 ICP1, PD7 INDEX
...
bool checkCRC() // crc check & restore sector to SD writable format
{
    ...
    if ((secData.crcHI == (crc >> 8)) && (secData.crcLO == (crc & 0xFF)))
...
int FloppyDrive::getSectorData(int lba)
{
    ...
    {
        if (flags.seclen == 2)
            n = disk_read_sector(secData.data, startSector + lba);
        else if (flags.seclen == 1)
            n = disk_read_sector(secData.data, startSector + (lba >> 1));
        else
            debugPrint_P(err_secSize);
    }
    else if (isVirtual())
...
int FloppyDrive::setSectorData(int lba)
{
    ...
    {
        if (flags.seclen == 2)
            n = disk_write_sector(secData.data, startSector + lba);
        else if (flags.seclen == 1)
            n = disk_write_sector(secData.data, startSector + (lba >> 1));
        else
            debugPrint_P(err_secSize);
    }
    else if (isVirtual())
...
void FloppyDrive::run()
{
    ...
    {
        SET_DSKCHANGE_LOW();
        if (isReady() || isVirtual())
            clrChanged(); // if a disk is loaded clear diskChange flag
    }
    ...
    while (!ds.isDrvChanged())
    {
        ...
        if (ds.isTrackChanged()) // if track changed
        {
            ...
            if (track < 0)
                track = 0; // Check if track valid
            else if (track >= numTrack)
                track = numTrack - 1;
        ...
        if (fdcWriteData(bitLength, wBuffer, (128 << flags.seclen) + 4)) // if WRITE_GATE asserted
        {
            ...
            if (res == 0)
            {
                ...
                debugPrint_P(err_readFDC); // couldnt read full sector
            while (IS_WRITE())
                ; // wait for WRITE_GATE to deassert
        }
        else
            fdcWriteGap(bitLength, 54);
        sector++; // next sector
        if (sector >= numSec)
            sector = 0;
    } // selected
