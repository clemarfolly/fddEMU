...
class driveStatus
{
  ...
  public:
    ...
    void clrTrackChanged()
    {
        f.trackChanged = 0;
    }
    uint8_t isTrackChanged()
    {
        return f.trackChanged;
    }
#if ENABLE_DRIVE_B // Dual drive mode
    uint8_t isDrive0()
    {
        return IS_SELECTA() ? (1 << 0) : 0;
    }
    uint8_t isDrive1()
    {
        return IS_MOTORA() ? (1 << 1) : 0;
    }
#else  // Single drive mode
    uint8_t isDrive0()
    {
        return IS_SELECTA() && IS_MOTORA() ? (1 << 0) : 0;
    }
    uint8_t isDrive1()
    {
        return 0;
    ...
    uint8_t getDriveSel()
    {
        return isDrive1() | isDrive0();
    }
    bool isDrvChanged()
    {
        if (f.driveSel == getDriveSel())
            return false;
        else
            f.driveSel = getDriveSel();
        return true;
    }
};
