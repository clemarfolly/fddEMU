...
void SerialUI::readRx()
{
    ...
    case 'S':
    case 's':
        if ((drv_sel == 0) || (drv_sel == DRIVE1))
            drv_sel = DRIVE0;
#if ENABLE_DRIVE_B
        else if (drv_sel == DRIVE0)
            drv_sel = DRIVE1;
    ...
    case 'N':
        ...
        Serial.write('>');
        Serial.print_P(str_selected);
        if (sdfile.getFileName()[0] != 0)
            Serial.print(sdfile.getFileName());
        else
            Serial.print_P(str_label);
        for (int8_t k = 0; k < 14; k++)
